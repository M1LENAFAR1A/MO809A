...
import numpy as np
import ray
import random # Vamos precisar para o comportamento malicioso
...
class ClienteGRPC:
    def __init__(self, cliente_id, client_type, dados_cliente):
    # Guarda as informações específicas deste cliente
        self.id = cliente_id
        self.type = client_type
        self.dados = dados_cliente

        # Prepara a conexão gRPC, que será a mesma para este cliente
        self.channel = grpc.insecure_channel('localhost:50051')
        self.stub = pb2_grpc.ModeloServiceStub(self.channel)

        print(f"Cliente Ator {self.id} (tipo: {self.type}) criado.")
